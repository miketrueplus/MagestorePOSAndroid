...
//        setParentController(mOrderHistoryListController);
    ...
    public void changeMaxStoreCreditRefund() {
        ...
        for (CartItem cart : mList) {
            total_item_price += ((cart.getBasePriceInclTax() - ((Math.abs(cart.getBaseDiscountAmount()) + Math.abs(cart.getBaseGiftVoucherDiscount()) + Math.abs(cart.getRewardpointsBaseDiscount()) / cart.getQtyOrdered()))) * cart.getQuantity());;
