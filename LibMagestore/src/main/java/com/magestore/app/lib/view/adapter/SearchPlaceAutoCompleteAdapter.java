...
public class SearchPlaceAutoCompleteAdapter<TModel extends Model> extends BaseAdapter implements Filterable {
     ...
     * @param service
     */
    public SearchPlaceAutoCompleteAdapter(Context context, CustomerService service) {
        mContext = context;
        mService = service;
    ...
    private List<TModel> search(Context context, String search) {
        try {
            return (List<TModel>) mService.placeAutoComplete(search);
        } catch (IOException e) {
            e.printStackTrace();
